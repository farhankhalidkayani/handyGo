...
class _FinanceBodyState extends State<FinanceBody> {
  ...
  RealtimeSubscription? _withdrawalSub;
  ...
  void initState() {
    ...
    _transactionSub!.stream.listen((_) => _load());
    _withdrawalSub = AppServices.realtime.subscribe([
      'databases.${HandyGoConfig.databaseId}.collections.${Collections.walletWithdrawals}.documents',
    ]);
    _withdrawalSub!.stream.listen((_) => _load());
  }

  @override
  void dispose() {
    _transactionSub?.close();
    _withdrawalSub?.close();
    super.dispose();
  }
  ...
  Future<void> _load() async {
    try {
      ...
      final withdrawals = await AppServices.transactions
  ...
  Widget build(BuildContext context) {
    ...
    final totalRevenue = _transactions.fold<double>(
      0,
      (sum, t) => sum + t.total,
    );
    final totalCommission = _transactions.fold<double>(
      0,
      (sum, t) => sum + t.commission,
    );
    final totalPaidToWorkers = _transactions.fold<double>(
      0,
            ...
            child: Wrap(
              ...
              children: [
                ...
                _StatCard(
                  ...
                  value: 'Rs. ${totalPaidToWorkers.toStringAsFixed(0)}',
                ),
                _StatCard(
