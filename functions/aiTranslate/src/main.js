// aiTranslate — §9.5: C8/A3/C7. Fires on messages.create. Translates for the recipient and
// runs a deterministic keyword scan for external-payment / abuse content — the scan never
// depends on the LLM tier so a flagged message is never missed just because Ollama is down.
...
module.exports = async ({ req, res, log, error }) => {
  ...
  if (!msg || !msg.$id) return res.json({ error: 'no message payload' }, 400);
