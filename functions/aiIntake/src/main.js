// aiIntake — §9.3: C2/C3/C4. Rules-first category match (keyword scan), LLM disambiguates
// when the rules engine is unsure (ambiguous text or Roman Urdu), then a deterministic
// price/duration band from the matched category. Never blocks on the LLM.
...
module.exports = async ({ req, res, log, error }) => {
  ...
  if (!problemText.trim()) return res.json({ error: 'problemText is required' }, 400);
  ...
  if (!categories.length) return res.json({ error: 'no service_categories seeded' }, 500);
...
Return JSON only: {"category":"...","confidence":0-1,"urgency":"low|normal|high|emergency","solution":"one-line advice in the user's language"}`;
    ...
    if (out.tier === 'llm') {
      ...
      log(`aiIntake: LLM tier failed, using rules/fallback — ${out.error}`);
    }
  }
