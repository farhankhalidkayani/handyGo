// routePlanner — §9.8 W5: ETA via OSRM (with a straight-line fallback if OSRM is
// unreachable, per the Risk Register §15), plus a rules-based schedule overlap check.
...
module.exports = async ({ req, res }) => {
  ...
  if (workerLat === undefined || workerLng === undefined) {
    return res.json({ error: 'workerLat/workerLng required' }, 400);
  }
  ...
  // Overlap check: any two scheduled jobs whose windows can't both be met given travel time
  const conflicts = [];
  ...
  return res.json({ route: ordered, conflicts });
};
