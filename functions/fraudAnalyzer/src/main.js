// fraudAnalyzer — §10.4/A5. Fires on fraud_reports.create. LLM summarises the evidence
// bundle and drafts a recommendation; admin always makes the final `adminDecision` (AI
// never writes it directly — governance rule in §8.3/A8).
...
module.exports = async ({ req, res, error }) => {
  ...
  const report = event.payload || event;
  if (!report || !report.$id) return res.json({ error: 'no report payload' }, 400);
