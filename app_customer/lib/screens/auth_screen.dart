...
/// Email-OTP login/signup — single flow for both, per plan §12 (createEmailToken creates the
/// Auth user on first use, so there's no separate "sign up" step).
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      final existing = await AppServices.profiles.findByAuthId(authUser.$id);
      if (!mounted) return;
