...
const _statusMeta = {
  BookingStatus.searchingWorkers: (
    label: 'Finding a worker',
    color: Colors.orange,
  ),
  BookingStatus.offersReceived: (
    label: 'Offers received',
    color: Colors.orange,
  ),
  BookingStatus.workerSelected: (
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    _activeBookingFuture = AppServices.bookings.findActiveForCustomer(
      widget.profile.id,
    );
    ...
    _bookingSub!.stream.listen((event) {
      ...
      if (booking.customerId != widget.profile.id) return;
      _refreshBookings();
    });
  }
  ...
  void _openPastBooking(Booking booking) {
    if (booking.status == BookingStatus.completed &&
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) =>
              RatingScreen(profile: widget.profile, bookingId: booking.id),
        ),
      );
    }
  }
  ...
  List<ServiceCategory> _recommendedCategories(
  ...
  Future<void> _openActiveBooking(Booking booking) async {
    if (booking.status == BookingStatus.completed &&
        booking.ratingGiven == null) {
      await Navigator.of(context).push(
        MaterialPageRoute(
      ...
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) =>
              TrackingScreen(profile: widget.profile, bookingId: booking.id),
        ),
      );
    }
    _refreshBookings();
  ...
  Future<void> _requestServiceManually() async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => ServiceRequestScreen(profile: widget.profile),
      ),
    );
    setState(() {
      _activeBookingFuture = AppServices.bookings.findActiveForCustomer(
              ...
              actions: [
                IconButton(
                  ...
                  onPressed: () => Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          SafetyCenterScreen(profile: widget.profile),
                    ),
                  ),
                ),
                IconButton(
                    ...
                    builder: (context, catSnapshot) {
                      ...
                      return FutureBuilder<List<Booking>>(
                        ...
                        builder: (context, historySnapshot) {
                          ...
                          final recommended = _recommendedCategories(
                            categories,
                            history,
                          );
                          return Column(
                            ...
                            children: [
                              ...
                              Builder(
                                builder: (context) {
                                  ...
                                  if (filtered.isEmpty)
                                    return const Text('No matching services.');
                                  return SizedBox(
                                    height: 92,
                                    child: ListView.separated(
                                      scrollDirection: Axis.horizontal,
                            ...
                            child: Card(
                              child: ListTile(
                                ...
                                leading: CircleAvatar(
                                  backgroundColor:
                                      (meta?.color ?? scheme.primary)
                                          .withValues(alpha: 0.15),
                                  child: Icon(
                                    _categoryIcon(b.problemText),
                                    color: meta?.color ?? scheme.primary,
                                    size: 20,
                                  ),
                                ),
                                title: Text(
                                ...
                                subtitle: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: (meta?.color ?? scheme.outline)
